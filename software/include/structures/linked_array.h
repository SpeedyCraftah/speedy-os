    ...
    class linked_array {
        public:
            ...
            T& get_at(uint32_t index) {
                // Safety check.
                if (index >= occupied) {
                    speedyos::speedyshell::printf("[FATAL] A linked array index was out of bounds.");
            ...
            void resize(uint32_t newCapacity) {
                if (newCapacity < occupied) {
                    speedyos::speedyshell::printf("[FATAL] A linked array was resized to a smaller capacity.");
                    speedyos::end_process(1);
                    __builtin_unreachable();
                }

                // Create a new storage area.
                entry* new_storage_ptr = (entry*)malloc(sizeof(entry) * newCapacity, true);
        ...
        private:
            ...
            entry* probe_index() {
                ...
                speedyos::speedyshell::printf("[FATAL] A linked array was full.");
                speedyos::end_process(1);
                __builtin_unreachable();
            }
