...
structures::string::string(char* str) {
    ...
    storage_ptr = (char*)malloc(sizeof(char) * _length);
...
structures::string& structures::string::concat(char src) {
    ...
    if (storage_ptr == nullptr) {
        speedyos::speedyshell::printf("[FATAL] Attempted to concat a nullptr string.");
    ...
    char* new_storage_ptr = (char*)malloc(srcLength + currentLength + 1);
    ...
    if (storage_ptr != nullptr) free(storage_ptr);
...
char& structures::string::operator[](int index) {
    if (storage_ptr == nullptr) {
        speedyos::speedyshell::printf("[FATAL] Overload operation attempted on an uninitialised string.");
        speedyos::end_process(1);
        __builtin_unreachable();
    }
...
char* structures::string::char_reference() {
    if (storage_ptr == nullptr) {
        speedyos::speedyshell::printf("[FATAL] Reference attempted on an uninitialised string.");
        speedyos::end_process(1);
        __builtin_unreachable();
    }
...
smart_ptr<char> structures::string::char_copy() {
    if (storage_ptr == nullptr) {
        speedyos::speedyshell::printf("[FATAL] Copy attempted on an uninitialised string.");
        speedyos::end_process(1);
        __builtin_unreachable();
    }

    auto str = smart_ptr<char>((char*)malloc(_length));
