...
class async {
    public:
        ...
        ReturnT value() {
            if (!complete) kernel::panic("A value has been requested in a promise while it has not been completed.");
            return return_value;
        }
        ...
        ~async() {
            ...
            if (param_ptr != nullptr && !complete) heap::free(param_ptr);
        }
    private:
